...
export function OrganizerCampaignsScreen() {
  ...
  const [organizerBuyerId, setOrganizerBuyerId] =
    useState<number>(DEFAULT_CREATED_BY);
  ...
  const loadData = useCallback(async () => {
    ...
    const [eventsResult, campaignsResult, usersResult, itemsResult] =
  ...
  useEffect(() => {
    ...
    const fallbackOrganizer = users.find(
      (userItem) => userItem.role === "organizer",
    );
  ...
  const handleSubmitCampaign = async () => {
    if (!selectedEventId) {
      setFormError(
        "Debes tener al menos un evento creado para registrar una campana.",
      );
      return;
    }
      ...
      <View className="flex-1 px-4 pt-6">
        ...
        {!isLoading && events.length === 0 ? (
          <Text className="mt-4 rounded-xl bg-[#fff6e8] px-3 py-2 text-sm text-[#8a5d13]">
            No hay eventos creados. Debes crear un evento para poder registrar
            campanas.
          </Text>
        ) : null}
            ...
            const progress =
              campaignItem.goalMoney > 0
                ? Math.min(
                    100,
                    Math.round(
                      (campaignItem.collectedMoney / campaignItem.goalMoney) *
                        100,
                    ),
                  )
                : 0;
                ...
                <View className="flex-row items-start justify-between">
                  <View className="flex-1 pr-3">
                    ...
                    <Text className="mt-1 text-xs text-[#5d7399]">
                      Evento:{" "}
                      {eventInfo
                        ? `${eventInfo.name} (${eventInfo.city})`
                        : `ID ${campaignItem.eventId}`}
                    </Text>
                  </View>
        ...
        >
          <View className="flex-1 justify-end bg-[#07163166]">
            <View className="max-h-[88%] rounded-t-3xl bg-white px-5 pt-5">
              ...
              >
                <Text className="text-lg font-extrabold text-[#17315c]">
                  Crear campana
                </Text>

                <Text className="mt-4 text-sm font-semibold text-[#27436d]">
                  Nombre
                </Text>
                <TextInput
                ...
                <Text className="mt-3 text-sm font-semibold text-[#27436d]">
                  Descripcion
                </Text>
                <TextInput
                  className="mt-1 rounded-xl border border-[#d3e2fb] bg-[#f8fbff] px-4 py-3 text-[#18335f]"
                ...
                {events.length === 0 ? (
                  <Text className="mt-3 rounded-xl bg-[#fff6e8] px-3 py-2 text-xs text-[#8a5d13]">
                    No hay eventos creados. Sin evento no se puede crear la
                    campana.
                  </Text>
                ) : null}
                ...
                <View className="mt-5 flex-row items-center justify-between">
                  ...
                  >
                    <Text className="font-semibold text-[#3a5176]">
                      Cancelar
                    </Text>
                  </Pressable>
                  <Pressable
