...
function getProgressValue(campaign: Campaign) {
  ...
  return Math.min(
    100,
    Math.round((campaign.collectedMoney / campaign.goalMoney) * 100),
  );
}
...
export function DonorCampaignsScreen() {
  ...
  const [moneyDraftByCampaign, setMoneyDraftByCampaign] = useState<
    Record<number, string>
  >({});
  const [donationFeedbackByCampaign, setDonationFeedbackByCampaign] = useState<
    Record<number, string>
  >({});
  const [isDonatingMoneyByCampaign, setIsDonatingMoneyByCampaign] = useState<
    Record<number, boolean>
  >({});

  const [selectedPhysicalItemByCampaign, setSelectedPhysicalItemByCampaign] =
    useState<Record<number, string>>({});
  const [physicalQuantityByCampaign, setPhysicalQuantityByCampaign] = useState<
    Record<number, string>
  >({});
  const [
    isPhysicalDropdownOpenByCampaign,
    setIsPhysicalDropdownOpenByCampaign,
  ] = useState<Record<number, boolean>>({});
  const [isDonatingItemsByCampaign, setIsDonatingItemsByCampaign] = useState<
    Record<number, boolean>
  >({});

  const [auctionItemDraftByCampaign, setAuctionItemDraftByCampaign] = useState<
    Record<number, string>
  >({});
  const [
    auctionDescriptionDraftByCampaign,
    setAuctionDescriptionDraftByCampaign,
  ] = useState<Record<number, string>>({});
  const [auctionPriceDraftByCampaign, setAuctionPriceDraftByCampaign] =
    useState<Record<number, string>>({});
  const [auctionFeedbackByCampaign, setAuctionFeedbackByCampaign] = useState<
    Record<number, string>
  >({});
  const [isCreatingAuctionByCampaign, setIsCreatingAuctionByCampaign] =
    useState<Record<number, boolean>>({});
  const [isBuyingAuctionById, setIsBuyingAuctionById] = useState<
    Record<number, boolean>
  >({});

  const [chatCampaignId, setChatCampaignId] = useState<number | null>(null);
  const [chatDraft, setChatDraft] = useState("");
  const [chatByCampaign, setChatByCampaign] = useState<
  ...
  const loadData = useCallback(async () => {
    ...
    const [campaignsResult, eventsResult, usersResult, itemsResult] =
    ...
    if (campaignsResult.status === "fulfilled") {
      ...
      failedSources.push(
        `campanas (${getErrorMessage(campaignsResult.reason)})`,
      );
      setCampaigns([]);
    }
    ...
    if (itemsResult.status === "fulfilled") {
      setItemDonations(
        normalizeCollection<ItemDonationResponse>(itemsResult.value),
      );
    } else {
    ...
    if (campaignsResult.status === "fulfilled") {
      await refreshCampaignAuctions(
        campaignsResult.value.data.map((campaign) => campaign.id),
      );
    } else {
  ...
  useEffect(() => {
    ...
    const matchedByEmail = users.find(
      (userItem) =>
        userItem.email.toLowerCase() === currentUser.email.toLowerCase(),
    );
  ...
  const inventoryByCampaign = useMemo(
    () => buildInventoryMap(itemDonations),
    [itemDonations],
  );

  const chatCampaign = useMemo(
    () =>
      campaigns.find((campaignItem) => campaignItem.id === chatCampaignId) ??
      null,
    [campaigns, chatCampaignId],
  );

  const chatMessages = chatCampaignId
    ? (chatByCampaign[chatCampaignId] ?? [])
    : [];
  ...
  useEffect(() => {
    ...
    const offChatMessageCreated = onChatMessageCreated<ChatMessageCreatedEvent>(
      (event) => {
        ...
        setChatByCampaign((prev) => {
          ...
          const optimisticIndex = bucket.findIndex((item) => {
            ...
            return (
              item.message.trim().toLowerCase() === normalizedIncomingMessage
            );
          });
          ...
          if (optimisticIndex >= 0) {
            ...
            nextBucket[optimisticIndex] = {
              id: nextId,
              author:
                event.authorName ?? `Usuario ${event.authorId ?? ""}`.trim(),
    ...
    const offAuctionCreated = onRealtime<AuctionRealtimeEvent>(
      "auction.created",
      handleAuctionRealtime,
    );
    const offAuctionUpdated = onRealtime<AuctionRealtimeEvent>(
      "auction.updated",
      handleAuctionRealtime,
    );
    const offAuctionSold = onRealtime<AuctionRealtimeEvent>(
      "auction.sold",
      handleAuctionRealtime,
    );

    const offInventoryUpdated = onRealtime<InventoryRealtimeEvent>(
      ...
      async (event) => {
        ...
        try {
          ...
          setItemDonations(
            normalizeCollection<ItemDonationResponse>(itemsResponse),
          );
        } catch {
  ...
  const handleDonateMoney = async (campaign: Campaign) => {
    const value = Number(
      (moneyDraftByCampaign[campaign.id] ?? "").replaceAll(/\D/g, ""),
    );
    ...
    try {
      ...
      setCampaigns((prev) =>
        prev.map((campaignItem) =>
          campaignItem.id === campaign.id
            ? {
                ...campaignItem,
                collectedMoney: campaignItem.collectedMoney + value,
              }
            : campaignItem,
        ),
      );
      ...
      setIsDonatingMoneyByCampaign((prev) => ({
        ...prev,
        [campaign.id]: false,
      }));
    }
  ...
  const handleDonatePhysicalItem = async (campaignId: number) => {
    const selectedItem =
      selectedPhysicalItemByCampaign[campaignId] ??
      PHYSICAL_DONATION_OPTIONS[0].key;
    const quantity = Number(
      (physicalQuantityByCampaign[campaignId] ?? "").replaceAll(/\D/g, ""),
    );
    ...
    try {
      ...
      const selectedLabel =
        PHYSICAL_DONATION_OPTIONS.find((option) => option.key === selectedItem)
          ?.label ?? selectedItem;
      ...
      setIsDonatingItemsByCampaign((prev) => ({
        ...prev,
        [campaignId]: false,
      }));
    }
  ...
  const handleCreateAuction = async (campaignId: number) => {
    ...
    const description = (
      auctionDescriptionDraftByCampaign[campaignId] ?? ""
    ).trim();
    const price = Number(
      (auctionPriceDraftByCampaign[campaignId] ?? "").replaceAll(/\D/g, ""),
    );
    ...
    try {
      ...
      setAuctionDescriptionDraftByCampaign((prev) => ({
        ...prev,
        [campaignId]: "",
      }));
      ...
      setIsCreatingAuctionByCampaign((prev) => ({
        ...prev,
        [campaignId]: false,
      }));
    }
      ...
      setCampaigns((prev) =>
        prev.map((campaignItem) =>
          campaignItem.id === campaignId
            ? {
                ...campaignItem,
                collectedMoney: campaignItem.collectedMoney + auctionPrice,
              }
            : campaignItem,
        ),
      );
  ...
  return (
    <View className="flex-1 bg-[#eef4ff]">
      <View className="flex-1 px-4 pt-6">
        ...
        >
          {campaigns.map((campaignItem, index) => {
            ...
            const physicalInventorySummary = PHYSICAL_DONATION_OPTIONS.map(
              (option) => ({
                label: option.label,
            ...
            const selectedItem =
              selectedPhysicalItemByCampaign[campaignItem.id] ??
              PHYSICAL_DONATION_OPTIONS[0].key;
            const selectedItemLabel =
              PHYSICAL_DONATION_OPTIONS.find(
                (option) => option.key === selectedItem,
              )?.label ?? selectedItem;
            const isDropdownOpen =
              isPhysicalDropdownOpenByCampaign[campaignItem.id] ?? false;
              ...
              >
                <View className="flex-row items-start justify-between">
                  <View className="flex-1 pr-3">
                    <Text className="text-base font-extrabold text-[#1b3259]">
                      {campaignItem.name}
                    </Text>
                  ...
                  <View className="items-end gap-2">
                    <View className="rounded-full bg-[#e9f1ff] px-3 py-1">
                      <Text className="text-xs font-bold text-[#1f4fb6]">
                        {progress}%
                      </Text>
                    </View>
                    <Pressable
                ...
                <View className="mt-3 flex-row gap-2">
                  <View className="flex-1 rounded-xl bg-[#edf3ff] p-3">
                    <Text className="text-xs font-semibold text-[#4b648d]">
                      Recaudado
                    </Text>
                ...
                {progress < 100 ? (
                  <View className="mt-3 flex-row items-center gap-2">
                    <TextInput
                      ...
                      onChangeText={(value) =>
                        setMoneyDraftByCampaign((prev) => ({
                          ...prev,
                          [campaignItem.id]: value,
                        }))
                      }
                    ...
                    >
                      <Text className="text-xs font-bold text-white">
                        {isDonatingMoneyByCampaign[campaignItem.id]
                          ? "Donando..."
                          : "Donar"}
                      </Text>
                    </Pressable>
                  ...
                  <Text className="mt-3 text-xs font-semibold text-[#1b7b45]">
                    Campana completada.
                  </Text>
                )}
                ...
                <View className="mt-3 rounded-xl bg-[#f6f9ff] p-3">
                  <Text className="text-xs font-semibold text-[#27436d]">
                    Donar articulos fisicos
                  </Text>
                  ...
                  >
                    <Text className="text-[#1f365d]">
                      Articulo: {selectedItemLabel}
                    </Text>
                  </Pressable>
                  ...
                  <View className="mt-2 flex-row items-center gap-2">
                    <TextInput
                      ...
                      keyboardType="number-pad"
                      onChangeText={(value) =>
                        setPhysicalQuantityByCampaign((prev) => ({
                          ...prev,
                          [campaignItem.id]: value,
                        }))
                      }
                    ...
                    >
                      <Text className="text-xs font-bold text-white">
                        {isDonatingItemsByCampaign[campaignItem.id]
                          ? "Donando..."
                          : "Donar articulo"}
                      </Text>
                    </Pressable>
                ...
                <View className="mt-3 rounded-xl bg-[#fff6e8] p-3">
                  <Text className="text-xs font-semibold text-[#8a5d13]">
                    Crear subasta solidaria
                  </Text>
                  <TextInput
                    className="mt-2 rounded-xl border border-[#f0d7b0] bg-white px-3 py-2 text-[#5f430f]"
                    onChangeText={(value) =>
                      setAuctionItemDraftByCampaign((prev) => ({
                        ...prev,
                        [campaignItem.id]: value,
                      }))
                    }
                  ...
                  <TextInput
                    ...
                    placeholderTextColor="#b89054"
                    value={
                      auctionDescriptionDraftByCampaign[campaignItem.id] ?? ""
                    }
                  />
                  <View className="mt-2 flex-row items-center gap-2">
                    <TextInput
                      ...
                      keyboardType="number-pad"
                      onChangeText={(value) =>
                        setAuctionPriceDraftByCampaign((prev) => ({
                          ...prev,
                          [campaignItem.id]: value,
                        }))
                      }
                    ...
                    >
                      <Text className="text-xs font-bold text-white">
                        {isCreatingAuctionByCampaign[campaignItem.id]
                          ? "Creando..."
                          : "Crear subasta"}
                      </Text>
                    </Pressable>
                ...
                <View className="mt-3 rounded-xl bg-[#fff9ef] p-3">
                  ...
                  {campaignAuctions.length === 0 ? (
                    <Text className="mt-1 text-xs text-[#9f7a3e]">
                      Aun no hay subastas en esta campana.
                    </Text>
                  ) : (
                    <View className="mt-2 gap-2">
                      {campaignAuctions.map((auction) => {
                        const seller = auction.sellerId
                          ? usersById.get(auction.sellerId)
                          : null;
                        const buyer = auction.buyerId
                          ? usersById.get(auction.buyerId)
                          : null;

                        return (
                          <View
                            className="rounded-xl border border-[#f0d7b0] bg-white px-3 py-3"
                            key={auction.id}
                          >
                            <Text className="text-sm font-bold text-[#6b4912]">
                              {auction.itemName}
                            </Text>
                            ...
                            {auction.status === "active" ? (
                              ...
                              >
                                <Text className="text-xs font-bold text-white">
                                  {isBuyingAuctionById[auction.id]
                                    ? "Comprando..."
                                    : "Comprar"}
                                </Text>
                              </Pressable>
          ...
          {!isLoading && !loadError && campaigns.length === 0 ? (
            <Text className="text-sm text-[#5d7498]">
              Aun no hay campanas publicadas.
            </Text>
          ) : null}
        </ScrollView>
      </View>
      ...
      {currentUser?.role === "donor" ? (
        <DonorBottomTabs activeTab="campanas" />
      ) : null}
    </View>
  );
}
